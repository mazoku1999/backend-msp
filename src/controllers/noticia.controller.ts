...
export const getNoticiasByCategoria = async (req: Request, res: Response) => {
    try {
...
export const getNoticiasPublicas = async (req: Request, res: Response) => {
    try {
        const [noticias] = await pool.execute(`
            SELECT 
                n.id,
...
export const getNoticiasPorCategoriaPublica = async (req: Request, res: Response) => {
    try {
        const { categoria } = req.params;
        const [noticias] = await pool.execute(`
            SELECT 
        ...
        console.error('Error al obtener noticias públicas por categoría:', error);
        return res.status(500).json({ message: 'Error en el servidor' });
    }
}; 