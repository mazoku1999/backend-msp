...
export const createVideo = async (req: AuthRequest, res: Response) => {
    try {
        ...
        res.status(201).json({
            ...
            slug,
            autor_id,
            estado: videoData.estado || 'borrador'
        });
    } catch (error) {
...
export const getVideosByCategoria = async (req: Request, res: Response) => {
    try {
...
export const getVideosPublicos = async (req: Request, res: Response) => {
    try {
        const [videos] = await pool.execute(`
            SELECT 
                v.id,
        ...
        console.error('Error al obtener videos públicos:', error);
        return res.status(500).json({ message: 'Error en el servidor' });
    }
};

export const getVideosPorCategoriaPublica = async (req: Request, res: Response) => {
    try {
        const { categoria } = req.params;
        const [videos] = await pool.execute(`
            SELECT 
        ...
        console.error('Error al obtener videos públicos por categoría:', error);
        return res.status(500).json({ message: 'Error en el servidor' });
    }
}; 